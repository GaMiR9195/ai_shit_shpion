...
(function () {
  ...
  const LANGS = ["txt", "js", "ts", "jsx", "tsx", "py", "go", "rust", "c", "cpp", "java", "sql", "sh", "css", "html", "json", "md"];
  ...
  const isBox = (o) => ["note", "sketch", "code", "image", "video", "file"].includes(o.type);
  ...
  function build(o) {
    ...
    el.addEventListener("pointermove", (e) => {
      ...
      if (dragging || drawMode) { el.style.cursor = ""; return; }
      const d = edgeAt(el, e);
    ...
    el.addEventListener("pointerdown", (e) => {
      ...
      if (drawMode) { e.preventDefault(); e.stopPropagation(); beginStroke(e); return; }
      if (e.ctrlKey || e.metaKey) { e.preventDefault(); e.stopPropagation(); select(o.id, true); return; }
  ...
  function paint(o) {
    ...
    if (o.type === "code") {
      ...
      if (o.md && document.activeElement !== ta) {
        view.innerHTML = window.MD.render(o.src || "", o.lang);
  ...
  /* One icon family: 24px line art, no plate, no fill. The set itself
     ...
     The old inline list only had ten entries, so most drops fell through
     to the generic cube. */
  const ICONS = FileIcons.ICONS;
  function iconKey(o) { return FileIcons.keyFor(o.name, o.mime); }
  ...
  async function handleFile(f, at) {
    if (READABLE(f) && f.size < 512 * 1024) {
      ...
      mkCode(at, { src, lang, name: f.name, h: codeHeight(src) });
      return;
    }
    ...
    const src = rec ? rec.url : await blobURL(f).catch(() => "");
  ...
  function startDrag(e, o, textEl) {
    ...
    closeMenu();
    const move = (ev) => {
      /* If the button is already up we missed the release — a video's own
         controls, an iframe or a lost capture can all swallow pointerup,
         and the card would then stay glued to the cursor. */
    ...
    const up = () => {
      ...
      if (moved) { save(); set.forEach(Sync.obj); }
  ...
  function pasteOwn(at) {
    ...
    } else if (c.pts) c.pts = c.pts.map((p) => ({ x: p.x + 24, y: p.y + 24, y: p.y + 24 }));
  ...
  function dropValue(text, at) {
    ...
    const n = mkNote(at);
    const first = t.split("\n")[0];
    if (t.includes("\n") || first.length > 42) { n.title = ""; n.body = t; }
    else { n.title = first; n.body = ""; }
    paint(n); save();
    return true;
  }
  ...
  function downloadObject(o) {
    ...
    const u = URL.createObjectURL(new Blob([o.src || ""], { type: "text/plain;charset=utf-8" }));
    const a = document.createElement("a");
  ...
  /* Several cards at once come down as one archive, packed at level 9.
     fflate is pulled in only at the moment you actually ask for a zip, so
     it costs nothing on load. */
  async function downloadZip(list) {
    ...
    for (const o of list) {
      ...
      if (!b) continue;
      entries[fileNameFor(o, used)] = b;
      n++;
    }
    if (!n) return toast("nothing to download");

    const { zip } = await import("https://cdn.jsdelivr.net/npm/fflate@0.8.2/esm/browser.js");
    const packed = await new Promise((res, rej) =>
      zip(entries, { level: 9, mem: 12 }, (err, data) => (err ? rej(err) : res(data))));

    const stamp = new Date().toISOString().slice(0, 10);
    const u = URL.createObjectURL(new Blob([packed], { type: "application/zip" }));
    const a = document.createElement("a");
    a.href = u; a.download = `space ${stamp}.zip`;
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(u), 4000);
    toast(`${n} files, zipped`);
  }
  ...
  function objectMenu(o, x, y) {
    ...
    if (VEC.includes(o.type)) {
      ...
      if (o.type === "curve") {
        items.push({ label: o.closed ? "Closed: on" : "Closed: off", run: () => { snapshot(); o.closed = !o.closed; drawVectors(); save(); } });
        items.push({ label: "Relax", run: () => { snapshot(); o.pts = G.autoNodes(G.smooth(G.resample(o.pts, Math.max(8, G.pathLength(o.pts) / 14)), 3), o.closed); drawVectors(); save(); } });
      }
  ...
  function toNote(o) {
    snapshot();
    const body = o.src || "";
    delete o.src; delete o.lang; delete o.md; delete o.wrap; delete o.name;
    o.type = "note"; o.title = ""; o.body = body; o.size = 13.5;
    render(); save(); select(o.id);
  }
  ...
  function startNodeDrag(o, kind, key) {
    ...
    const up = () => {
      ...
      removeEventListener("pointermove", move);
      removeEventListener("pointerup", up);
      if (moved) { fuseNode(o, kind, key); save(); }
    };
    addEventListener("pointermove", move);
    addEventListener("pointerup", up);
  }
  ...
  ink.addEventListener("pointerdown", (e) => {
    if (e.button !== 0 || drawMode || tidyMode) return;
    const t = e.target;
  ...
  app.addEventListener("pointerdown", (e) => {
    ...
    if (drawMode) { e.preventDefault(); beginStroke(e); return; }
    if (e.ctrlKey || e.metaKey) { e.preventDefault(); return startMarquee(e); }
  ...
  addEventListener("keydown", (e) => {
    ...
    if (mod && e.key === "0") { e.preventDefault(); camT.x = 0; camT.y = 0; camT.z = 1; kick(); return; }
    if (mod && (e.key === "=" || e.key === "+" || e.key === "-")) {
      e.preventDefault();
      const k = e.key === "-" ? 1 / 1.18 : 1.18;
    ...
    if (k === "n") { e.preventDefault(); mkNote(at); }
    if (k === "s") { e.preventDefault(); mkSketch(at); }
    if (k === "c") { e.preventDefault(); mkCode(at); }
    if (k === "d") { e.preventDefault(); setDraw(!drawMode); }
    if (k === "g") { e.preventDefault(); setTidy(!tidyMode); }
    if (k === "a" && !mod) {
      e.preventDefault();
