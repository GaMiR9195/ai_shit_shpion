...
(function (g) {
  ...
  function silhouetteArrow(pts, hullPts, diag) {
    ...
    for (const [A, B] of axes(pts, hullPts)) {
      ...
      // rebuild the two barbs from the head's own corners, so the drawn
      // head is reproduced as two open strokes. Still never a triangle.
      const heading = Math.atan2(B.y - A.y, B.x - A.x);
      const bx = A.x + (B.x - A.x) * (m / BINS), by = A.y + (B.y - A.y) * (m / BINS);
      const nx = -pr.uy, ny = pr.ux;
      const barbs = [1, -1].map((s) => {
        const cx = bx + nx * (headW / 2) * s, cy = by + ny * (headW / 2) * s;
        return {
          len: Math.round(Math.hypot(cx - B.x, cy - B.y) * 10) / 10,
          ang: Math.round(norm(Math.atan2(cy - B.y, cx - B.x) - heading) * 1000) / 1000,
        };
      });
      best = { kind: "arrow", a: A, b: snapAngle(A, B), head: { barbs } };
    }
    return best;
  }
  ...
  function recognize(raw, opts) {
    ...
    if (dev < 0.13 && corners.length === 0 && ringy > 0.86 && ringy < 1.2 && turn.signed > Math.PI * 1.5) {
      /* A hand that was already close to round gets a true circle. Only a
         visibly oval stroke stays an oval. */
      const round = dev < 0.09 && Math.abs(bb.w - bb.h) < Math.max(bb.w, bb.h) * 0.2;
  ...
  function arrowAt(pts, L, first) {
    ...
    if (dev > Math.max(6, chord * 0.16)) return null;
    ...
    for (const pk of peaks) {
      ...
      if (off < 0.35 || off > 1.31) return null;   // 20°..75° off the reversed shaft
    ...
    const head = {
      barbs: barbs.map((x) => ({ len: Math.round(x.len * 10) / 10, ang: Math.round(x.ang * 1000) / 1000 })),
    };
    return { kind: "arrow", a: shaft[0], b: snapAngle(shaft[0], tip), head };
  }
  ...
  function headStrokes(tip, heading, head, scale) {
    ...
    if (!barbs) {
      const len = 13 * s;
      barbs = [{ len, ang: Math.PI * 0.82 }, { len, ang: -Math.PI * 0.82 }];
    ...
    return barbs.map((b) => {
      ...
      return { from: tip, to: P(tip.x + Math.cos(a) * b.len, tip.y + Math.sin(a) * b.len) };
    });
  }
  ...
  g.G = { P, dist, add, sub, mul, lerp, pathLength, bbox, segDist, rdp, smooth, resample,
    ...
    cornerIdx, regularize, turning, headStrokes, findArrow,
